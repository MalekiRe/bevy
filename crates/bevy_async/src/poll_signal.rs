...
/// PollSignal is a custom signaling primitive used in order to fufill our specific requirements for
...
/// This also makes replacing the poll signal automatically drop and signal the previous one.
#[derive(Clone)]
#[cfg(feature = "std")]
...
impl PollSignal {
    ...
    pub(crate) fn new() -> Self {
        #[cfg(feature = "std")]
        {
            PollSignal(Arc::new((Mutex::new(false), std::sync::Condvar::new())))
        }
        #[cfg(not(feature = "std"))]
        {
            PollSignal(Arc::new(Mutex::new(false)))
        }
    }

    /// Waits until another cloned instance of `PollSignal` has been dropped.
    /// If any cloned instance of `PollSignal` is dropped then this wait stops waiting.
    #[cfg(feature = "std")]
    #[inline]
